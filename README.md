...
### 1. GitHub and GitHub Container Registry (GHCR)

The repository includes a GitHub Actions workflow in `.github/workflows/docker-publish.yml` that builds multi-architecture images (`linux/amd64` and `linux/arm64`) on every push to `main`.
...
1. Initialize Git and push your repository:
   ...
   git branch -M main
   git remote add origin https://github.com/YOUR_GITHUB_USERNAME/fnack.git
   git push -u origin main
   ```

2. After the GitHub Action finishes:
   - Go to your GitHub profile -> **Packages** -> **fnack**.
   - Under **Package Settings**, set visibility to **Public**.

3. Pull and run the image on any system:
   ```bash
   docker pull ghcr.io/YOUR_GITHUB_USERNAME/fnack:latest
   ```
...
```bash
docker login
docker build -t YOUR_DOCKERHUB_USERNAME/fnack:latest .
docker push YOUR_DOCKERHUB_USERNAME/fnack:latest
```

### 3. Offline Tarball Transfer
...
# Transfer
scp fnack_image.tar.gz docker-compose.yml user@remote-host:~/fnack/
